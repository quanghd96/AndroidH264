...
void
DisplayYUV_16(unsigned int *pdst1, unsigned char *y, unsigned char *u, unsigned char *v, int width,
              int height, int src_ystride, int src_uvstride, int dst_ystride) {
    int i, j;
    int r, g, b, rgb;
...
/*
 * Class:     com_quang_androidh264_H264Decoder
 * Method:    initDecoder
 * Signature: ()I
 */
jint Java_com_quang_androidh264_H264Decoder_initDecoder(JNIEnv *env, jobject thiz, jint width,
                                                        jint height) {
    iWidth = width;
    iHeight = height;
 ...
 * Signature: ()I
 */
jint Java_com_quang_androidh264_H264Decoder_uninitDecoder(JNIEnv *env, jobject thiz) {
    if (c) {
        decode_end(c);
        free(c->priv_data);
 ...
 * Signature: ([B[I)I
 */
jint Java_com_quang_androidh264_H264Decoder_decoderNal(JNIEnv *env, jobject thiz, jbyteArray in,
                                                       jint nalLen, jbyteArray out) {
    int i;
    int imod;
    int got_picture;
    ...
    if (consumed_bytes > 0) {
        DisplayYUV_16((int *) Pixel, picture->data[0], picture->data[1], picture->data[2], c->width,
                      c->height, picture->linesize[0], picture->linesize[1], iWidth);
/*
